...
<%
	if(conn.getResponseCode() == HttpURLConnection.HTTP_OK) {  
		...
		if(xmlResponse.contains("<BinaryPayload") || xmlResponse.contains("<wmts:BinaryPayload")) {
			...
			imageUrl = "base64decoder.jsp?mimetype=" + URLEncoder.encode(payload.format, "UTF-8") + "&dataAttributeName=sip_data";
			url = "xmlResponse.jsp?xmlAttributeName=sip_xml";
			
			// put the image data and the xml in the session 
			request.getSession().setAttribute("sip_data",payload.payloadContent);	
			request.getSession().setAttribute("sip_xml",xmlResponse);
		} else {
			url = "errorResponse.jsp?errorAttributeName=sip_error&format=text/xml";
			
			// put the image data and the xml in the session 
			request.getSession().setAttribute("sip_error",xmlResponse);
		}
%>		
...
%>
<h1><u>SIP Response: <%=conn.getResponseCode()%></u></h1>
<%
		...
		String responseUrl = "errorResponse.jsp?errorAttributeName=sip_error";
		
		// put the image data and the xml in the session 
		request.getSession().setAttribute("sip_error",errorResponse);
%>
